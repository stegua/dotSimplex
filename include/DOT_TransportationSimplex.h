...
#ifndef LEMON_MYNETWORK_SIMPLEX_H
#define LEMON_MYNETWORK_SIMPLEX_H
 ...
 private:
   ...
   class BlockSearchPivotRule {
    ...
    public:
      ...
      // Constructor
      BlockSearchPivotRule(MyNetworkSimplex &ns) :
         _source(ns._source), _target(ns._target),
         _cost(ns._cost), _state(ns._state), _pi(ns._pi),
         _in_arc(ns.in_arc), _arc_num(ns._arc_num), _dummy_arc(ns._dummy_arc),
         ...
         _block_size = std::max(int(BLOCK_SIZE_FACTOR *
                                    std::sqrt(double(_arc_num-_dummy_arc))),
                                MIN_BLOCK_SIZE);
      }
...
search_end:
 ...
 public:
   ...
   MyNetworkSimplex(int node_num, bool arc_mixing = true) :
      _node_num(node_num), _arc_num(0),
   ...
   int num_arcs() const {
      return _source.size() - _dummy_arc;
   }

   void addNode(int i, Value b) {
      _supply[i] = b;
   }
   ...
   int updateArcs(const Vars& as) {
      ...
      // Memorizza il primo arco inserito, per far ripartire il pricing da li.
      for (; idx < idx_max; ++idx) {
         while (e < e_max) {
            // Replace useless variables with new variables
            if (_state[e] == STATE_LOWER && (_cost[e] + _pi[_source[e]] - _pi[_target[e]] > 1e-09))
               break;
            ++e;
         }
         if (e >= e_max)
   ...
   Cost potential(int n) const {
      return _pi[n];
   }

   void shuffle(int seed = 13) {
      ...
      for (int i = _arc_num - _dummy_arc - 1; i > 0; --i) {
         int j = D(g, param_t(0, i));
         std::swap(_source[_dummy_arc + i], _source[_dummy_arc + j]);
         std::swap(_target[_dummy_arc + i], _target[_dummy_arc + j]);
         std::swap(_cost[_dummy_arc + i], _cost[_dummy_arc + j]);
      }
   }
